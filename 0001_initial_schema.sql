...
DO $$ BEGIN
    CREATE TYPE shipment_status AS ENUM ('PENDING', 'ASSIGNED', 'PICKED_UP', 'DELIVERED', 'CANCELLED');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_shipments_airports ON shipments(origin_airport, destination_airport);
...
COMMENT ON TABLE shipments IS 'Luggage delivery shipments with full lifecycle tracking';

COMMENT ON COLUMN shipments.status IS 'PENDING: awaiting driver, ASSIGNED: claimed by driver, PICKED_UP: luggage collected, DELIVERED: completed, CANCELLED: cancelled';
COMMENT ON COLUMN shipments.price_cents IS 'Price stored in cents to avoid floating point issues';
COMMENT ON COLUMN shipments.origin_airport IS '3-letter airport code or OTH for custom address';
COMMENT ON COLUMN shipments.destination_airport IS '3-letter airport code or OTH for custom address';
COMMENT ON COLUMN shipments.notes IS 'Contains customer info, luggage details, distance, promo codes, and internal notes';
